...
- offgrid              → 进入交互式 REPL
- offgrid "修复bug"    → 单次执行
- offgrid init         → 初始化 OFFGRID.md
- offgrid memory       → 查看项目记忆
"""
...
_log_dir = Path.home() / ".offgrid"
_log_dir.mkdir(parents=True, exist_ok=True)
_file_handler = logging.FileHandler(_log_dir / "offgrid.log", encoding="utf-8")
_file_handler.setFormatter(logging.Formatter("%(asctime)s %(name)s %(levelname)s: %(message)s"))
logging.getLogger("offgrid").addHandler(_file_handler)
logging.getLogger("offgrid").propagate = False
logging.getLogger("offgrid").setLevel(logging.DEBUG)
...
def _build_pipeline(model_path, ollama_url, ollama_model, project_root, verbose):
    ...
    from offgrid.registry import ExpertRegistry
    ...
    from offgrid.search.duckduckgo import _searxng_available, _try_start_searxng, _get_searxng_url
    import offgrid.search.duckduckgo as _search_mod
    ...
    from offgrid.cli.onboarding import load_config as _load_cfg
    ...
    from offgrid.experts.debug_subagent import DebugSubagent
...
def _run_task(task, gate, orchestrator, memory, project_root, verbose, plan=False, no_search=False):
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"),
        ...
        try:
            ...
            if gate_result.get("needs_search") and not no_search and et not in ("chat", "office"):
                try:
                    from offgrid.search.query_generator import QueryGenerator
                    from offgrid.search.duckduckgo import search as ddg_search
    ...
    if result["success"]:
        ...
        if et == "chat":
            ...
            console.print(f"\n  {reply}" if reply else
                          "\n  你好！我是OffGrid，专注于代码任务。有什么代码问题需要帮忙吗？")
            return True
...
def _render_header(model: str, project_root: str, registry=None):
    ...
    for line in _KAIWU_SHADOW:
        console.print(line)
    console.print(f"  [dim]离线编码  v{VERSION}[/dim]")
...
def _repl(model_path, ollama_url, ollama_model, project_root, verbose):
    ...
    from offgrid.memory.offgrid_md import OffgridMemory
    from offgrid.core.sysinfo import get_sysinfo, VRAMWatcher
    from offgrid.core.context_pruner import ContextPruner
    from offgrid.cli.status_bar import StatusBar, TokPerSecEstimator
    ...
    from offgrid.core.model_capability import detect_model_tier, get_strategy, tier_display_name
    ...
    while True:
        ...
        if user_input.startswith("/"):
            ...
            if cmd in ("/exit", "/quit", "/q"):
                ...
                # Generate OFFGRID.md template
                from offgrid.core.offgrid_md import generate_offgrid_template
                result = generate_offgrid_template(project_root)
                ...
                if not arg:
                    ...
                    from offgrid.cli.onboarding import load_config, _save_config
                    cfg = load_config()
    ...
    try:
        from offgrid.memory.session_md import save_session
        tasks_done = []
...
def _handle_multi_with_tasks(tasks, gate, orchestrator, project_root, console):
    ...
    from offgrid.core.task_compiler import TaskCompiler
    from rich.progress import Progress, SpinnerColumn, TextColumn
...
def _maybe_show_weekly_stats(console):
    ...
    last_shown_path = _Path.home() / ".offgrid" / "last_stats_shown.txt"
    now = _time.time()
...
def _handle_api_command(parts: list[str], current_url: str, current_model: str):
    ...
    from offgrid.cli.onboarding import load_config, _verify_api, _save_config, CONFIG_PATH
...
def main(
    ...
    if verbose:
        ...
        logging.getLogger("offgrid").propagate = True
    ...
    if do_init:
        from offgrid.core.offgrid_md import generate_offgrid_template
        console.print(f"  {generate_offgrid_template(project_root)}")
        from offgrid.memory.offgrid_md import OffgridMemory
        console.print(OffgridMemory().init(project_root))
        return
    ...
    from offgrid.cli.onboarding import is_first_run, run_onboarding, load_config
...
def cmd_memory(
    ...
    if reset:
        import shutil
        offgrid_dir = os.path.join(project_root, ".offgrid")
        if os.path.isdir(offgrid_dir):
            shutil.rmtree(offgrid_dir)
            console.print(f"  [green]已清空 {offgrid_dir}[/green]")
        else:
            console.print(f"  [dim]没有记忆文件需要清空[/dim]")
        return
    content = OffgridMemory().show(project_root)
    Console().print(Panel(content, title="KAIWU.md", border_style="blue"))
...
def expert_export(
    ...
    """导出专家为 .kwx 包。"""
    from offgrid.registry.expert_packager import ExpertPackager
    reg = _get_registry()
    try:
...
def cmd_status(
    ...
    console.print(Panel(
        ...
        f"项目: {project_root}\n"
        f"记忆: {'[green]KAIWU.md 已初始化[/green]' if has_memory else '[yellow]未初始化 (offgrid init)[/yellow]'}",
        title="KwCode Status",
        border_style="cyan",
    ))
...
def cmd_stats(
    ...
    console.print()
    console.print(f"  [bold]OffGrid 价值报告[/bold]  过去{days}天")
    console.print("  " + "─" * 45)
...
def cmd_serve_mcp(
    ...
    from offgrid.mcp.router_mcp import KaiwuMCP
...
def checkpoint_list():
    """查看所有快照。"""
    from offgrid.core.checkpoint import list_checkpoints
...
def checkpoint_restore():
    """还原到最近快照。"""
    from offgrid.core.checkpoint import restore_latest
    if restore_latest():
...
def cmd_setup_search():
    ...
    try:
        ...
        if r.returncode != 0:
            ...
            console.print("  请先启动 Docker Desktop，然后重新运行 offgrid setup-search")
            return
        ...
        console.print()
        console.print("  安装后重新运行 [bold]offgrid setup-search[/bold]")
        return
    except subprocess.TimeoutExpired:
        console.print("  [red]✗ Docker 响应超时[/red]")
        return

    container_name = "offgrid-searxng"
    ...
    console.print("  offgrid 启动时会自动检测并使用。")
    console.print()
    console.print("  [dim]管理命令：[/dim]")
    console.print("  [dim]  停止：docker stop offgrid-searxng[/dim]")
    console.print("  [dim]  启动：docker start offgrid-searxng[/dim]")
    console.print("  [dim]  卸载：docker rm -f offgrid-searxng[/dim]")
