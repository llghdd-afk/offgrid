...
class ChatExpert:
    ...
    def _needs_search(self, ctx: TaskContext) -> bool:
        user_input = ctx.user_input.strip()
        _REALTIME_KEYWORDS = ["今天", "今日", "明天", "本周", "这周", "现在", "最新", "最近", "天气", "温度", "价格", "股价"]
        if any(kw in user_input for kw in _REALTIME_KEYWORDS):
            return True
        _FOLLOWUP_PATTERNS = ["穿什么", "怎么去", "那个呢", "还有呢", "然后呢", "具体说", "详细", "举个例", "比如", "展开", "为什么", "什么意思", "怎么理解"]
        if len(user_input) < 20 and any(p in user_input for p in _FOLLOWUP_PATTERNS):
            logger.debug("[chat] follow-up detected, skip search")
            return False
        _REASONING_PATTERNS = ["建议", "合适", "应该", "怎么选", "哪个好", "优缺点", "对比", "区别", "适合", "推荐", "注意什么", "需要注意", "有什么技巧"]
    ...
    def _run_chat(self, ctx: TaskContext) -> dict:
        try:
            reply = self.llm.generate(prompt=ctx.user_input, system=self._build_system(ctx, CHAT_SYSTEM), max_tokens=200, temperature=0.7)
            ...
            fallback = "你好！我是OffGrid，专注于代码任务。有什么代码问题需要帮忙吗？"
    ...
    def _run_with_search(self, ctx: TaskContext) -> dict:
        try:
            ...
            if search_result and len(search_result) > 30:
                ...
                reply = self.llm.generate(prompt=prompt, system=self._build_system(ctx, CHAT_SEARCH_SYSTEM), max_tokens=500, temperature=0.3)
                ctx.generator_output = {"explanation": reply.strip(), "patches": []}
                return {"passed": True, "output": reply.strip()}
        except Exception as e:
    ...
    def _run_search_fail(self, ctx: TaskContext) -> dict:
        try:
            reply = self.llm.generate(prompt=ctx.user_input, system=self._build_system(ctx, CHAT_SEARCH_FAIL_SYSTEM), max_tokens=300, temperature=0.3)
            ctx.generator_output = {"explanation": reply.strip(), "patches": []}
            return {"passed": True, "output": reply.strip()}
        except Exception as e:
