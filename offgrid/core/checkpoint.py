...
Git repos use git stash; non-git repos copy files to ~/.offgrid/checkpoints/.
...
CHECKPOINT_DIR = Path.home() / ".offgrid" / "checkpoints"
STASH_PREFIX = "offgrid-checkpoint"
...
class Checkpoint:
    ...
    def _file_copy(self, files: list[str]) -> bool:
        """Non-git fallback: copy files to ~/.offgrid/checkpoints/."""
        try:
