...
class PipelineOrchestrator:
    ...
    def run(
        ...
        ctx = TaskContext(
            ...
            offgrid_memory=self.memory.load(project_root),
        ...
        # ── OFFGRID.md rules injection ──
        offgrid_sections = load_offgrid_md(project_root)
        if offgrid_sections:
            offgrid_rules = build_offgrid_system(expert_type, offgrid_sections)
            if offgrid_rules:
                ctx.offgrid_rules = offgrid_rules
                # Prepend to expert_system_prompt so it flows to all experts
                if ctx.expert_system_prompt:
                    ctx.expert_system_prompt = f"{offgrid_rules}\n\n{ctx.expert_system_prompt}"
                else:
                    ctx.expert_system_prompt = offgrid_rules
        ...
        if expert_type == "chat":
            ...
            if self.chat_expert:
                ...
                ctx.generator_output = {"explanation": "我是OffGrid，专注于代码任务。", "patches": []}
                result = {"passed": True}
    ...
    def _do_review(self, ctx: TaskContext, on_status) -> dict:
        ...
        try:
            from offgrid.experts.reviewer import ReviewerExpert
    ...
    def _persist_reflection(self, project_root, ctx, gate_result, success):
        ...
        try:
            if not ctx.reflection:
                return
            from offgrid.memory.pattern_md import save_reflection
            save_reflection(
                project_root=project_root,
