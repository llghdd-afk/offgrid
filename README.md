...
| 日期 | 内容 |
|------|------|
| 05-18 | ReAct多轮推理循环 + 精准Retry(接近通过时聚焦重试) + Eval基准测试标准化(24道bench tasks) |
...
**理论来源**：ReAct（Yao et al., ICLR 2023）——推理+行动交替执行，每步看到真实环境反馈，显著优于纯生成模式。

当 Gate 检测到复杂任务（重构、多文件迁移、需要探索的任务）时，自动切换到 ReAct 模式：

```
用户："重构认证模块"
  └─► Gate 判定 complexity=hard + 重构关键词 → 路由到 ReAct
        └─► Step 1: read_file 读取认证相关文件
        └─► Step 2: grep 搜索所有调用点
        └─► Step 3: write_file 修改核心模块
        └─► Step 4: run_test 验证
        └─► Step 5: grep 确认无遗漏引用
        └─► Step 6: submit 提交方案
```

LLM 自主决定每步做什么，看到真实反馈后调整策略。简单任务仍走原有确定性流水线。

### 原理九：精准 Retry（接近通过时聚焦重试）

当测试只差 1-2 个通过时（≥50% 通过 + ≤2 个失败），触发精准重试模式：

- 保留完整诊断信息（800字符，非默认100字符截断）
- 注入聚焦指令："只修复上面报告的失败测试，不要改动其他代码"
- 最小化修改策略：只改必须改的行，其余保持原样
